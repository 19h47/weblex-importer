    ...
    mounted() {
        if (this.dataRss) {
            ...
            for (let element of tempData) {
                ...
                const oldQuestion =
                    "document.getElementById('page-fiche').classList.add('quizz-open'); return false;";
                const newQuestion =
                    "document.getElementById('reponse" +
                    idNumber +
                    "').classList.add('quizz-open');document.getElementById('reponse" +
                    idNumber +
                    "').classList.remove('quizz-close');document.getElementById('rna" +
                    idNumber +
                    "').classList.add('quizz-close');document.getElementById('rna" +
                    idNumber +
                    "').classList.remove('quizz-open');return false;";
                ...
                const replacement = element.description
                    .replaceAll(oldTarget, newTarget)
                    .replaceAll(oldID, newID)
                    .replaceAll(oldID2, newID2)
                    .replaceAll(oldQuestion, newQuestion)
